...
describe('/visits', () => {
  ...
  describe('PUT /:id', () => {
    ...
    it('should return 200 if new visit start date at most 7 days later', async () => {
      ...
      expect(body).toStrictEqual({
        ...
        visitParts: visit.visitParts.map((visitPart) => ({
          ...
          endDate: advanceDateByMinutes(visitPart.endDate, newOldStartDateDifference).toISOString(),
          employee
        }))
      });
    });
    ...
    it('should return 400 if new visit start date is at least 7 days later', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toStrictEqual({
        message: 'New visit start date should be at most 7 days later from the initial one'
      });
    });
    ...
    it('should return 400 if visit date was already changed', async () => {
      ...
      await request(app).put(`/visits/${visit.id}`).send({ startDate: '2024-01-04T14:00:00.000Z' });
      ...
      expect(status).toBe(400);
      expect(body).toStrictEqual({
        message: 'Cannot change the date of a visit because it has been already changed'
      });
    });
  });
  ...
  describe('PUT /:id/cancel', () => {
    ...
    it('should return 200 and visit with half-reduced costs if cancelled at most 24h after now', async () => {
      ...
      const { status, body } = await request(app).put(`/visits/${visit.id}/cancel`);
      ...
      visit.visitParts.forEach((visitPart) => {
        expect(Scheduler.getInstance().cancelJob).toHaveBeenCalledWith(`${visitPart.id}`);
      });
    });
